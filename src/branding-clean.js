...
const aliases={
  candy:['candy'],lava:['lava'],galaxy:['galaxy'],yinyang:['yinyang','yinying','yin yang','yinyangbase'],radioactive:['radioactive','radiation','radioactivebase'],cursed:['cursed','crused','cursedbase'],rainbow:['rainbow'],diamond:['diamond'],divine:['divine','divinebase'],cyber:['cyber','cyberbase'],phantom:['phantom','phantombase'],crystal:['crystal','crystalbase'],
  '10m250m':['moneypuggy','moneypug','money','cashpuggy','richpuggy','moneyemoji'],
  '250m500m':['moneypuggy','moneypug','cashpuggy','richpuggy','money'],
  '500m1b':['moneypuggy','moneypug','cashpuggy','richpuggy','money','rich'],
  '1b5b':['moneypuggy','moneypug','cashpuggy','richpuggy','money','rich'],
  '5b':['moneypuggy','moneypug','cashpuggy','richpuggy','money','rich'],
  'ograreitems':['meowl','og','rare','ogitem','rareitem']
};
...
async function updateMiddlemanOrderPanel(client){
  try{
    const guildId=process.env.GUILD_ID;
    const guild=guildId?client.guilds.cache.get(guildId):null;
    if(!guild)return;
    const channel=guild.channels.cache.find(c=>c.isTextBased?.()&&c.name==='🤝・middleman');
    if(!channel)return;
    const messages=await channel.messages.fetch({limit:20});
    const panel=messages.find(m=>m.author.id===client.user.id&&m.components.length);
    if(!panel)return;
    const embed=new EmbedBuilder().setTitle(MIDDLEMAN_TITLE).setDescription(MIDDLEMAN_DESCRIPTION).setColor(0x5865f2).setImage(BRAND_IMAGE_URL).setFooter({text:'POWERED BY FSMM'});
    await panel.edit({embeds:[embed]});
    console.log('[FSMM] Middleman order panel updated automatically');
...
EmbedBuilder.prototype.setTitle=function(title){
  if(title==='🤝 FSMM MIDDLEMAN'||title==='🎫 FSMM MIDDLEMAN TICKET'){
    this.__fsmmMiddleman=true;
...
EmbedBuilder.prototype.setDescription=function(description){
  if(this.__fsmmMiddleman)return originalSetDescription.call(this,MIDDLEMAN_DESCRIPTION);
...
EmbedBuilder.prototype.setFooter=function(data){
  if(this.__fsmmMiddleman)return originalSetFooter.call(this,{text:'POWERED BY FSMM'});
  if(data&&typeof data==='object'&&typeof data.text==='string'&&(/^FSMM\s*[•|·-]\s*v?\d/i.test(data.text.trim())||data.text.trim()==='FSMM'))return this.setImage(BRAND_IMAGE_URL);
  return originalSetFooter.call(this,data);
};

const originalAddOptions=StringSelectMenuBuilder.prototype.addOptions;
StringSelectMenuBuilder.prototype.addOptions=function(...args){const patch=option=>{if(Array.isArray(option))return option.map(patch);if(!option||typeof option!=='object'||!option.label||option.emoji)return option;const emoji=findEmoji(option.label);return emoji?{...option,emoji}:option;};return originalAddOptions.apply(this,args.map(patch));};
