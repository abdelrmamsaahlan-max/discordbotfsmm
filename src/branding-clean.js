...
function loadGuildEmojis() {
  ...
  const req = https.get(`https://discord.com/api/v10/guilds/${guildId}/emojis`, { headers: { Authorization: `Bot ${token}` } }, res => {
    const chunks = [];
    res.on('data', c => chunks.push(c));
    res.on('end', () => {
      try {
        if (res.statusCode !== 200) throw new Error(`Discord emoji API HTTP ${res.statusCode}`);
        for (const emoji of JSON.parse(Buffer.concat(chunks).toString('utf8'))) {
          if (emoji?.name && emoji?.id) emojiByName.set(normalize(emoji.name), { id: emoji.id, name: emoji.name, animated: Boolean(emoji.animated) });
        }
        console.log(`[FSMM] Loaded ${emojiByName.size} server emojis for menus`);
      } catch (e) { console.error('[FSMM EMOJIS]', e.message); }
    });
  });
...
function findEmoji(label) {
  const key = normalize(label), candidates = aliases[key] || [key];
  for (const candidate of candidates) if (emojiByName.has(normalize(candidate))) return emojiByName.get(normalize(candidate));
  for (const [name, emoji] of emojiByName) if (candidates.some(candidate => name === normalize(candidate) || name.includes(normalize(candidate)))) return emoji;
}

const originalSetFooter = EmbedBuilder.prototype.setFooter;
EmbedBuilder.prototype.setFooter = function(data) {
  if (data && typeof data === 'object' && typeof data.text === 'string' && /^FSMM\s*[•|·-]\s*v?\d/i.test(data.text.trim())) return originalSetFooter.call(this, { ...data, text: 'FSMM', iconURL: FOOTER_ICON_URL });
  if (data && typeof data === 'object' && data.text === 'FSMM') return originalSetFooter.call(this, { ...data, iconURL: data.iconURL || FOOTER_ICON_URL });
...
// Keep the Discord presence as: Watching FSMM.
const originalLogin = Client.prototype.login;
Client.prototype.login = async function(...args) {
  const result = await originalLogin.apply(this, args);
  try {
    this.user?.setPresence({ activities: [{ name: 'FSMM', type: ActivityType.Watching }], status: 'online' });
    console.log('[FSMM] Presence set to Watching FSMM');
  } catch (e) { console.error('[FSMM PRESENCE]', e.message); }
  return result;
};
