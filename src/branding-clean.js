...
// Ticket role routing. The bot finds the roles by name, so the role IDs do not need to be hard-coded.
...
function findTicketSpecificRole(guild,type,description){if(type==='middleman'){const key=middlemanValueFromText(description);if(!key)return null;return roleByPatterns(guild,MIDDLEMAN_ROLE_ALIASES[key]);}if(type==='base-painting'){const base=baseFromText(description);if(!base)return null;const n=normalize(base);return roleByPatterns(guild,[`${base} Index Provider`,`${base} Provider`,`${base} Index`,`${n}indexprovider`,`${n}provider`]);}if(type==='support')return roleByPatterns(guild,['FSMM Staff','Staff']);return null;}
...
TextChannel.prototype.send=async function(payload){try{const topic=this.topic||'';const firstEmbed=Array.isArray(payload?.embeds)?payload.embeds[0]:null;const title=firstEmbed?.data?.title||firstEmbed?.title||'';if(topic.includes('FSMM_USER:')&&topic.includes('TYPE:')&&String(title).startsWith('🎫 FSMM ')){const type=(topic.match(/TYPE:([^\s]+)/)||[])[1]||'support';const description=firstEmbed?.data?.description||firstEmbed?.description||'';const role=findTicketSpecificRole(this.guild,type,description);const roleMention=role?` <@&${role.id}>`:'';const content=String(payload.content||'');payload={...payload,content:content+roleMention,allowedMentions:{...(payload.allowedMentions||{}),roles:Array.from(new Set([...(payload.allowedMentions?.roles||[]),...(role?[role.id]:[])]))},embeds:[...(payload.embeds||[]),ticketInfoEmbed(type,description,role)]};console.log(`[FSMM] Ticket routing: ${type} -> ${role?.name||'no matching role found'}`);}}catch(e){console.error('[FSMM TICKET ROUTING]',e.message);}return originalTextChannelSend.call(this,payload);};
