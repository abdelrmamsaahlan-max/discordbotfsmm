const { Client, REST, Routes, SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionFlagsBits, ModalBuilder, TextInputBuilder, TextInputStyle, ChannelType } = require('discord.js');
...
async function configuredLogs(guild) {
  return guild.channels.cache.find(c => c.type === ChannelType.GuildText && typeof c.topic === 'string' && c.topic.includes(LOG_MARKER)) || guild.channels.cache.find(c => c.type === ChannelType.GuildText && c.name === 'fsmm-logs') || null;
}
...
Client.prototype.login = async function (...args) {
  if (!this.__fsmmFinalLoginHook) {
    ...
    this.once('clientReady', async () => {
      ...
      try {
        const cmd = new SlashCommandBuilder().setName('setlogs').setDescription('Set the channel where FSMM logs are sent').addChannelOption(o => o.setName('channel').setDescription('Log channel').addChannelTypes(ChannelType.GuildText).setRequired(true)).setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).toJSON();
...
async function finalInteractionHandler(i) {
  try {
    ...
    const type = typeFromTopic(i.channel.topic);
    const uid = userFromTopic(i.channel.topic);
    const team = teamRole(type);
    ...
    if (i.customId === 'fsmm_claim') {
      ...
      try {
        await i.channel.setTopic(`${i.channel.topic} CLAIMED_BY:${i.user.id}`.slice(0, 1024));
    ...
    if (i.customId === 'fsmm_adduser') {
      mark(i);
      if (!isStaff(i) && !hasRole(i, team)) return i.reply({ content: '❌ Team members only.', ephemeral: true });
      return i.showModal(new ModalBuilder().setCustomId('fsmm_adduser_modal').setTitle('Add User to Ticket').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('user').setLabel('User ID or mention').setPlaceholder('123456789012345678').setStyle(TextInputStyle.Short).setRequired(true))));
    }
    ...
    if (i.customId === 'fsmm_close' && !isStaff(i)) {
      mark(i);
      return i.reply({ content: '❌ Only FSMM Staff/Mod/Owner can close tickets.', ephemeral: true });
    }
  } catch (e) {
    console.error('[FSMM FINAL]', e.stack || e.message);
    if (!i.replied && !i.deferred) { mark(i); await i.reply({ content: '❌ Something went wrong. Please try again.', ephemeral: true }).catch(() => {}); }
  }
}
...
Client.prototype.on = function (event, listener) {
  ...
  if (!this.__fsmmInteractionRouter) {
    ...
    this.__fsmmInteractionListeners = [finalInteractionHandler];
    originalOn.call(this, 'interactionCreate', async i => {
      for (const fn of this.__fsmmInteractionListeners) {
        if (i.replied || i.deferred || i.__fsmmHandledByFinal) break;
        try { await fn(i); } catch (e) { console.error('[FSMM INTERACTION]', e.stack || e.message); }
      }
    });
  }
  this.__fsmmInteractionListeners.push(async i => {
    if (i.__fsmmHandledByFinal || i.replied || i.deferred) return;
    return listener(i);
  });
  return this;
};
...
console.log('[FSMM FINAL] clean interaction router loaded');
