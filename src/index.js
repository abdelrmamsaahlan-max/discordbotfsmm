...
function isOwner(interaction) {
  return Boolean(interaction.member?.roles?.cache?.some(r => normalize(r.name) === normalize(OWNER)));
}
...
function findBaseEmoji(guild, base) {
  const aliases = {
    yinyang:['yinyang','yinyangbase','yinyangmutation'],
  ...
  const names = aliases[normalize(base)] || [normalize(base)];
...
function embed(title, description) {
  return new EmbedBuilder().setTitle(title).setDescription(description).setColor(0x5865f2).setFooter({text:`FSMM • v${VERSION}`});
}
...
function cleanUsername(user) {
  return user.username.toLowerCase().replace(/[^a-z0-9]/g,'').slice(0,45) || 'user';
}
...
async function upsertPanel(channel,e,components) {
  const messages = await channel.messages.fetch({limit:20}).catch(() => null);
  const panel = messages?.find(m => m.author.id === client.user.id && m.components.length > 0);
  if(panel) return panel.edit({embeds:[e],components:[components]});
  return channel.send({embeds:[e],components:[components]});
}

async function setup(guild) {
  const {cat} = await getRoles(guild);
  const oldNames = ['🎫・ticket-center','ticket-center','fsmm-ticket-panel','🎫・tickets','🎟️・tickets'];
  for(const c of guild.channels.cache.filter(x => x.type === ChannelType.GuildText && oldNames.includes(x.name)).values()) await c.delete('FSMM removed legacy ticket center').catch(() => {});
  ...
  for(const [name,e,component] of panelConfigs(guild)) {
    let channel = guild.channels.cache.find(c => c.type === ChannelType.GuildText && c.name === name && c.parentId === cat.id);
    if(!channel) {
      channel = await guild.channels.create({name,type:ChannelType.GuildText,parent:cat.id,permissionOverwrites:[{id:guild.roles.everyone.id,allow:[PermissionFlagsBits.ViewChannel],deny:[PermissionFlagsBits.SendMessages]}],reason:'FSMM service panel'});
    }
...
function helpEmbed() {
  return embed('🤖 FSMM BOT COMMANDS',[
    '**Everyone:**',
    ...
    '`/botinfo` — Bot version and uptime.',
    '',
    '**Owner only:**',
    '`/setup` — Repair/create the FSMM panels.',
    '`/ticket` — Rebuild the FSMM ticket panels.'
  ].join('\n'));
}
...
client.once('ready', async () => {
  ...
  try {
    ...
    console.log(`[FSMM ${VERSION}] COMMANDS REGISTERED: /ping /help /serverinfo /userinfo /avatar /membercount /botinfo /setup /ticket`);
    const guild = client.guilds.cache.get(GUILD_ID);
    if(!guild) return console.error(`[FSMM ${VERSION}] GUILD ${GUILD_ID} NOT FOUND`);
    await setup(guild);
    console.log(`[FSMM ${VERSION}] PANELS VERIFIED/UPDATED AUTOMATICALLY`);
  } catch(error) {
    console.error(`[FSMM ${VERSION}] READY ERROR`,error);
  }
});
  ...
  try {
    if(i.isChatInputCommand()) {
      if(i.commandName === 'ping') {
        const sent = await i.reply({content:'🏓 Checking...',fetchReply:true,flags:MessageFlags.Ephemeral});
      ...
      if(i.commandName === 'serverinfo') {
        const g=i.guild;
        const e=embed(`🏠 ${g.name}`,'Server information');
      ...
      if(i.commandName === 'userinfo') {
        ...
        const e=embed(`👤 ${user.username}`,'Member information');
        e.setThumbnail(user.displayAvatarURL({size:512}));
        e.addFields(
      ...
      if(i.commandName === 'avatar') {
        const user=i.options.getUser('user') || i.user;
        const e=embed(`🖼️ ${user.username}'s Avatar`, 'Full-size avatar');
        e.setImage(user.displayAvatarURL({size:1024,dynamic:true}));
        return i.reply({embeds:[e],flags:MessageFlags.Ephemeral});
      }
      ...
      if(i.commandName === 'botinfo') {
        ...
        const e=embed('🤖 FSMM BOT INFO','Reliable FSMM utility + service ticket bot.');
        e.addFields({name:'Version',value:VERSION,inline:true},{name:'Uptime',value:`${d}d ${h}h ${m}m ${s}s`,inline:true},{name:'API Latency',value:`${client.ws.ping}ms`,inline:true});
        return i.reply({embeds:[e],flags:MessageFlags.Ephemeral});
      }
      if(i.commandName === 'setup' || i.commandName === 'ticket') {
        if(!(await ownerOnly(i))) return;
        await i.deferReply({flags:MessageFlags.Ephemeral});
        await setup(i.guild);
        return i.editReply('✅ **FSMM panels verified successfully.**\n\n🤝 Middleman\n🛟 Support\n🎨 Mutation Base Painting\n\nNo ticket center or legacy trade-ticket panel was created.');
      }
    }
    ...
    if(i.isStringSelectMenu()) {
      ...
      if(i.customId === 'mutation_pick') {
        ...
        return i.reply({content:`🎨 **${base} Base** selected. This preview is only visible to you.`,embeds:[paintPreview(base,i.guild)],components:[row(new ButtonBuilder().setCustomId(`paint_start:${encodeURIComponent(base)}`).setLabel('Start Base Painting Ticket').setStyle(ButtonStyle.Primary))],flags:MessageFlags.Ephemeral});
      }
    }
    ...
    if(i.isButton()) {
      ...
      if(i.customId === 'fsmm_close') {
        const canClose=i.memberPermissions?.has(PermissionFlagsBits.ManageChannels) || i.member?.roles?.cache?.some(r => normalize(r.name) === normalize(STAFF));
        if(!canClose) return i.reply({content:'❌ Only FSMM Staff or members with Manage Channels can close tickets.',flags:MessageFlags.Ephemeral});
    ...
    console.error(`[FSMM ${VERSION}] INTERACTION ERROR`,error);
    const msg={content:'❌ Something went wrong while processing that request. Please try again.',flags:MessageFlags.Ephemeral};
...
client.login(TOKEN).catch(error => {
  console.error(`[FSMM ${VERSION}] LOGIN FAILED`,error);
  process.exit(1);
});
