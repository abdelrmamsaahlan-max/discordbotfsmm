...
// Discord select options support custom server emojis. We find the emoji by name
// so the emojis the owner already created in the server are used automatically.
...
function findBaseEmoji(guild, base) {
  ...
  const aliases = {
    'yinyang': ['yinyang', 'yin_yang', 'yin-yang'],
  ...
  const emoji = guild.emojis.cache.find(e => names.some(n => normalize(e.name) === normalize(n)));
...
function mutationMenu(guild) {
  return row(new StringSelectMenuBuilder()
    .setCustomId('mutation_pick')
    .setPlaceholder('🎨 Select a mutation base...')
    .addOptions(mutationBases.map(base => ({
      label: base,
      value: base,
      description: `${base} Base • mutation painting`,
      emoji: findBaseEmoji(guild, base)
    }))));
}
...
async function createTicket(interaction, type, d={}) {
  ...
  e.addFields(
    ...
    {name:'Service',value:type === 'basepainting' ? 'Base Painting' : type[0].toUpperCase()+type.slice(1),inline:true}
  );
  ...
  if(d.base) {
    e.addFields({name:'Base Requested',value:d.base,inline:true});
    e.setImage(baseImage(d.base));
  }
  ...
  try {
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (i.customId === 'mutation_pick') {
        ...
        return i.reply({
          content:`🎨 **${base} Base** selected. This preview is only visible to you.`,
          embeds:[paintPreview(base, i.guild)],
          components:[row(new ButtonBuilder().setCustomId(`paint_start:${encodeURIComponent(base)}`).setLabel('Start Base Painting Ticket').setStyle(ButtonStyle.Primary))],
          ephemeral:true
        });
      }
    }
    ...
    if (i.isModalSubmit()) {
      if (i.customId.startsWith('mm_modal:')) {
        ...
        return createTicket(i,'middleman',{
          value,
          other:i.fields.getTextInputValue('other'),
          trade:i.fields.getTextInputValue('trade'),
          tip:i.fields.getTextInputValue('tip') || ''
        });
      }
      ...
      if (i.customId.startsWith('paint_modal:')) {
        ...
        return createTicket(i,'basepainting',{
          base,
          roblox:i.fields.getTextInputValue('roblox'),
          notes:i.fields.getTextInputValue('notes') || ''
        });
