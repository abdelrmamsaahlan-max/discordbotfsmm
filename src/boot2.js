...
if(!source.includes("require('./giveaway')")){const pos=source.indexOf(marker);if(pos<0)throw new Error('FSMM giveaway patch: login marker not found.');source=source.slice(0,pos)+"\nrequire('./giveaway')({client,store,saveStore,env:{TOKEN,CLIENT_ID,GUILD_ID}});\n"+source.slice(pos);}
