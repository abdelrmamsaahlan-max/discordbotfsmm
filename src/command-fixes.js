...
Client.prototype.login=function(...args){const client=this;if(!client.__fsmmCommandFixes){client.__fsmmCommandFixes=true;client.once('clientReady',async()=>{try{const rest=new REST({version:'10'}).setToken(TOKEN);const current=await rest.get(Routes.applicationGuildCommands(CLIENT_ID,GUILD_ID));const map=new Map((current||[]).filter(c=>!['announce','transcript','exist'].includes(c.name)).map(c=>[c.name,c]));for(const c of commands)map.set(c.name,c);await rest.put(Routes.applicationGuildCommands(CLIENT_ID,GUILD_ID),{body:[...map.values()]});console.log(`[FSMM COMMAND FIXES] cleaned /exist and ensured /calculator (${map.size} total)`);}catch(e){console.error('[FSMM COMMAND FIXES]',e.message);}});}return originalLogin.apply(this,args);};
